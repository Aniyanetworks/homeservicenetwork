            ...
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                {NETWORKING_AUDIENCE.map((item) => (
                  <span
                    key={item}
                    className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm font-semibold text-white/80 transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-amber-500/40 hover:bg-brand-amber-500/10"
                  >
                    {item}
                  </span>
                ))}
          ...
          <Reveal from="right">
            <ul className="grid gap-3">
              {NETWORKING_HIGHLIGHTS.map((point) => (
                <li
                  key={point}
                  className="flex items-start gap-3 rounded-xl border border-white/10 bg-white/5 p-4 transition-colors duration-200 hover:bg-white/10"
                >
                  <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-brand-amber-500 text-navy-950">
